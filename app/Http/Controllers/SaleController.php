...
class SaleController extends Controller
{
    ...
    $customers = Customer::select('id', 'name', 'status')
        ...
        $difference = $cashDrawer && $cashDrawer->closing_balance !== null
            ? (float) $cashDrawer->closing_balance - $expectedBalance
            : null;
    ...
    public function store(Request $request)
    {
        ...
        if (!$todayDrawer || $todayDrawer->status !== 'open') {
            return back()->withErrors([
                'cash_drawer' => 'Opening balance is required before creating invoices.',
            ]);
        }
