          ...
          <div class="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
            <div class="p-3 bg-white rounded-lg border border-gray-200">
              <p class="text-xs text-gray-600">
                ...
                >
                  (per {{ unitLabel(product?.sales_unit, product?.sales_unit_id) }})
                </span>
              </p>
              <p class="text-base font-bold text-green-600">
                {{ formatPrice(purchasePricePerSalesUnit) }}
              </p>
...
const basePurchasePrice = computed(() => {
  const batchPrice = Number(fetchedBatchPrice.value);
  if (Number.isFinite(batchPrice) && batchPrice > 0) {
    return batchPrice;
  }

  const productPrice = Number(props.product?.purchase_price);
  return Number.isFinite(productPrice) ? productPrice : 0;
});

const salesUnitsPerPurchaseUnit = computed(() => {
  const purchaseToTransferRate = Number(props.product?.purchase_to_transfer_rate) || 0;
  const transferToSalesRate = Number(props.product?.transfer_to_sales_rate) || 0;
...
const purchasePricePerSalesUnit = computed(() => {
  const basePrice = basePurchasePrice.value;
...
const purchasePriceFormulaText = computed(() => {
  const purchaseUnitName = unitLabel(props.product?.purchase_unit, props.product?.purchase_unit_id) || "Purchase Unit";
  const salesUnitName = unitLabel(props.product?.sales_unit, props.product?.sales_unit_id) || "Sales Unit";
  const conversionRate = salesUnitsPerPurchaseUnit.value;

  if (conversionRate <= 0) {
    return `Base: ${formatPrice(basePurchasePrice.value)} per ${purchaseUnitName}`;
  }

  return `Base: ${formatPrice(basePurchasePrice.value)} per ${purchaseUnitName} (1 ${purchaseUnitName} = ${conversionRate} ${salesUnitName})`;
});
